...
Эълон беришдан олдин фойдаланувчи ПРОФИЛИНИ текширади:
  1. Nickname (username) "user" сўзи билан бошланиши (одатий/шубҳали андоза)
  2. Ўз исми, фамилияси, nickname'и (username) ва bio'сида 18+ мазмун
  3. Профилга бириктирилган ШАХСИЙ КАНАЛ (Telegram'нинг "Personal chat"
     функцияси) номи/username'ида 18+ мазмун
Учаласи ҳам bot.get_chat(user_id) орқали БИТТА сўровда олинади.
...
from database import (
    ...
    starts_with_generic_username, block_for_suspicious_username,
)
...
async def check_profile_and_maybe_block(user_id: int, ad_id: int = None) -> bool:
    ...
    # ═══ 1. NICKNAME "user" БИЛАН БОШЛАНАДИМИ? ═══
    ...
    if starts_with_generic_username(own_username):
        logging.warning(
            "Шубҳали nickname аниқланди: user_id=%s (@%s) — блокланди",
            user_id, own_username
        )
        await block_for_suspicious_username(user_id, own_username, ad_id=ad_id)
        return True

    # ═══ 2. ЎЗ ИСМ / NICKNAME / BIO — 18+ ═══
