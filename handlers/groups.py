...
async def on_bot_membership_changed(event: types.ChatMemberUpdated):
    ...
    just_joined = is_in and not was_in
    just_promoted = is_in and is_admin and not was_admin
    ...
    if just_joined and not is_admin:
        # ⚠️ Ҳали администратор эмасмиз — аммо кўп гуруҳда оддий аъзо
        # сифатида ҳам ёза оламиз, шунинг учун аввал ГУРУҲНИНГ ЎЗИГА
        # админ ҳуқуқи сўраб ёзишга ҳаракат қиламиз (_ask_for_admin_rights
        # ичида). Фақат гуруҳда «фақат админлар ёзади» чекловидан хато
        # чиқса — ботни қўшган одамга ШАХСИЙ ёзамиз. Админ ҳуқуқи
        # берилиши биланоқ (қуйидаги just_promoted тармоғи) созлашни
        # ўзимиз бошлаймиз.
        await _ask_for_admin_rights(event)
        return

    if just_joined or just_promoted:
        # Администраторимиз — энди гуруҳга хавфсиз ёза оламиз.
        await _send_group_welcome(event)
        return
...
async def _ask_for_admin_rights(event: types.ChatMemberUpdated):
    """
    Гуруҳга: админ ҳуқуқи сўраб хабар юборади. Кўп гуруҳда «Фақат
    админлар хабар юбора олади» чеклови ёқилган бўлади — шунда бот
    (ҳали оддий аъзо сифатида) ёзолмайди, шу ҳолатда ботни қўшган
    одамга ШАХСИЙ чатда ёзамиз (акс ҳолда сабабини ҳеч ким билмайди).
    """
    text = (
        f"👋 Ассалому алайкум, ҳурматли аъзолар! Мени шу гуруҳга "
        f"қўшганингиз учун катта раҳмат! 🐄🐑🐐\n\n"
        f"Мол бозори эълонларини гуруҳингизга жойлаш, бозор таҳлили "
        f"ва бошқа фойдали маълумотлар билан ўртоқлашиш учун менга "
        f"шу гуруҳда <b>администратор</b> ҳуқуқи бериб қўйишингизни "
        f"илтимос қиламан (камида «Хабар юбориш» рухсати билан). 🙏\n\n"
        f"✅ Ҳуқуқ берилиши биланоқ, ортиқча ташвишланманг — қолганини "
        f"ўзим давом эттираман: гуруҳга ёзаман ва қайси вилоят(лар) "
        f"эълонлари керак эканини сўрайман. Ишонч билдирганингиз учун "
        f"раҳмат! 🤝"
    )
    ...
    try:
        ...
        logging.info(
            f"Гуруҳга (chat_id={event.chat.id}) админ ҳуқуқи ҳақида "
            f"ёзилмади (эҳтимол «фақат админлар ёзади» чекловидан): {e}"
        )

    # ═══ Гуруҳга ёзиб бўлмади — ботни қўшган одамга ШАХСИЙ чатда ═══
    try:
        await bot.send_message(
            chat_id=event.from_user.id,
            text=(
                f"👋 Ассалому алайкум! Мени «{html.escape(event.chat.title or '')}» "
                f"гуруҳингизга қўшганингиз учун катта раҳмат! 🐄🐑🐐\n\n"
                f"Гуруҳда «фақат администраторлар хабар юбора олади» "
                f"чеклови борга ўхшайди — шунинг учун бу сўровни сизга "
                f"шахсан юборяпман.\n\n"
                f"Мол бозори эълонларини гуруҳингизга жойлаш, бозор таҳлили "
                f"ва бошқа фойдали маълумотлар билан ўртоқлашиш учун менга "
                f"шу гуруҳда <b>администратор</b> ҳуқуқи бериб қўйишингизни "
                f"илтимос қиламан (камида «Хабар юбориш» рухсати билан). 🙏\n\n"
                f"✅ Ҳуқуқ берганингиздан сўнг, ортиқча ташвишланманг — "
                f"қолганини ўзим давом эттираман: гуруҳга ёзаман ва қайси "
                f"вилоят(лар) эълонлари керак эканини сўрайман. Ишонч "
                f"билдирганингиз учун раҳмат! 🤝"
            ),
            parse_mode="HTML"
        )
    except Exception as e:
        # Одам бот билан ҳали шахсий чат бошламаган бўлиши мумкин
        logging.info(
            f"Ботни қўшган одамга ({event.from_user.id}) админ ҳуқуқи "
            f"ҳақида ёзилмади: {e}"
        )


async def _send_group_welcome(event: types.ChatMemberUpdated):
    """Гуруҳга танишув хабари + вилоят танлаш тугмалари."""
    # Гуруҳ аллақачон созланган бўлса (масалан бот админликдан олиниб,
    # кейин қайта админ қилинган) — бутун онбордингни такрорламаймиз.
    existing_regions = await get_regions_for_chat(event.chat.id)
    if existing_regions:
        try:
            await bot.send_message(
                chat_id=event.chat.id,
                text=(
                    "✅ Администратор ҳуқуқи учун раҳмат — ишни давом эттираман.\n"
                    f"Бу гуруҳ боғланган вилоят(лар): "
                    f"<b>{html.escape(', '.join(existing_regions))}</b>\n\n"
                    "Ўзгартириш учун: /viloyat"
                ),
                parse_mode="HTML"
            )
        except Exception as e:
